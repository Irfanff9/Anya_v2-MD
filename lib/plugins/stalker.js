...
anya({
    ...
    if (args.length < 1) return pika.reply(`*${Config.themeemoji} Example:* ${prefix + command} 3.irpann_kidss\n\n> Bot will fetch details about the specified Instagram user.`);
...
${data.biography}`;
        ...
        .catch(err => {
            console.error(err);
            pika.reply("❌ User Not Found or Unable to Fetch Data!");
        });
});
...
anya({
    ...
    if (args.length < 1) return pika.reply(`*${Config.themeemoji} Example:* ${prefix + command} Irfanff9\n\n> Bot will fetch details about the specified GitHub user.`);
