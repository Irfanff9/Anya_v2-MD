...
exports.smsg = async (anyaV2, pika, store) => {
    ...
    if (pika.key) {
        pika.id = pika.key.id
	    const checkBot = (
    (pika.id.startsWith('BAES') ||
     pika.id.startsWith('BAE5') ||
     pika.id.startsWith('3EBO') ||
     pika.id.startsWith('3EB0') ||
     pika.id.startsWith('ANYAWEB') ||
     pika.id.startsWith('SHIZOWEB') ||
     pika.id.includes('BAES') ||
     pika.id.includes('BAE5') ||
     pika.id.includes('3EBO') ||
     pika.id.includes('3EB0') ||
     pika.id.includes('ANYAWEB') ||
     pika.id.includes('SHIZOWEB')) &&
    pika.id.length >= 12
);
        pika.isBot = checkBot
    	pika.isBaileys = checkBot
